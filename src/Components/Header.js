            ...
            <div className="modal__body-top">
              <div className="modal__body-top-info">
                <div className="content">
                  <div className={`register ${currentComp === 'Register' ? 'active' : ''} `} onClick={() => setCurrentComp('Register')}>
                    Register
                  </div>
                  <div className={`login ${currentComp === 'Login' ? 'active' : ''}`} onClick={() => setCurrentComp('Login')}>
                    Login
                  </div>
                </div>
              </div>
    ...
    return (
      <div className="header__options-auth">
        <div className="header__options-auth--web">
          <div className="balance">
            <span className="amount">
              NGN{showBalance ? betAmount : '*****'}
              <span
                className={`visible ${showBalance ? '' : 'hide-balance'}`}
                onClick={() => {
          ...
          <div className="deposit" onClick={() => showCurrentComp('deposit', true)}>
            <span>Deposit</span>
          </div>
          <div className="uk-button-group button-group">
            <button className="uk-button uk-button-default uk-button-small header__button">
              &nbsp;
            ...
            <div className="uk-inline header__button-open">
              <button className="uk-button uk-button-default uk-button-small header__button header__button-open-on" type="button">
        ...
        <div className="header__options-auth--mobile">
          ...
          <button className="uk-button uk-button-default uk-button-small header__options-auth--mobile-button" onClick={() => showCurrentComp('deposit', true)}>
            Deposit
          </button>
    ...
    <nav className="uk-navbar-container uk-margin header" uk-navbar="true">
      ...
      <div className="uk-navbar-left header__body">
        <div className="header__body-left">
          ...
          <Link className="uk-navbar-item uk-logo header__body-left-logo uk-padding-remove" href="#" to="/">
            <Logo />
          </Link>
        </div>
              ...
              <div className="header__options-web">
                <form onSubmit={handleSubmit}>
                  <div className="form-content">
                    <input className="uk-input uk-form-width-medium header__input" type="text" {...getFieldProps('email')} placeholder="Email" />
                    {error ? <span className="error">{error}</span> : null}
                    <span className="error">{touched['email'] && errors['email']}</span>
                  </div>
                  <div className="form-content">
                    <input className="uk-input uk-form-width-small header__input" type="password" {...getFieldProps('password')} placeholder="Password" />
                    <span className="error">{touched['password'] && errors['password']}</span>
                  </div>

                  <button
                    className={`uk-button uk-button-default uk-button-small header__button ${!errors.email && !errors.password && (touched['email'] || touched['password']) ? 'active' : ''}`}
                    type="submit"
                  >
                    Log in
                  </button>
                </form>
                <button className="uk-button uk-button-default uk-button-small header__register" onClick={() => showCurrentComp('userForm', true)}>
                  Register
                </button>
