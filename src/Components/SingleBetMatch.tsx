...
export const SingleBetMatch = ({bet}: ownProp) => {
  ...
  const showViewMore = () => {
    if (bet.selectedMatches.length > 4) {
      return viewLength !== bet.selectedMatches.length ? (
        <div className="view-all" onClick={() => setViewLength(bet.selectedMatches.length)}>
          view all...
        </div>
      ) : (
        <div className="view-all" onClick={() => setViewLength(4)}>
          view less...
        </div>
      )
    }
    ...
    <>
      <div className="cashout__match">
        ...
        <div className="cashout__match-selections">
          {bet.selectedMatches.map((match, index) =>
            index < viewLength ? (
              <React.Fragment key={match.match_id}>
                <span key={match.match_id} title={`${match.home_team} v ${match.away_team}`}>
                  {shortenText(`${match.home_team} v ${match.away_team}`, 0, shortenTextLen)} <span className="market">{match.market}</span>
                </span>{' '}
                <br />
              </React.Fragment>
            ) : (
